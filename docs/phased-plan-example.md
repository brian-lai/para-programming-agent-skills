...
1. The agent reads `context/context.md` and detects phased plan
...
The agent (or user) works through the to-dos:
...
1. The agent analyzes changes in worktree: `git -C .para-worktrees/implement-user-authentication-phase-1 diff main...HEAD`
...
1. The agent checks that phase 1 is "completed"
...
User/The agent implements API layer, commits incrementally, summarizes phase 2, creates PR, merges.
