...
if echo "$body" | grep -q '"phased_execution"'; then echo "FAIL inline phased_execution JSON literal present"; fail=1; else echo "PASS no inline phased_execution JSON literal"; fi
exit "$fail"
