...
frontmatter=$(awk '/^---$/{c++; if (c==2) exit; next} c==1' skills/para-archive/SKILL.md)
...
fail=0
[ "$name" = "para-archive" ] && echo "PASS name=para-archive" || { echo "FAIL name=$name"; fail=1; }
...
[ "$model" = "haiku" ] && echo "PASS top-level model=haiku" || { echo "FAIL model=$model"; fail=1; }
[ "$effort" = "low" ] && echo "PASS top-level effort=low" || { echo "FAIL effort=$effort"; fail=1; }
